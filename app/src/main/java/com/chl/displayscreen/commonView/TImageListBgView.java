    ...
    private void initHorizontalScrollView() {
        ...
        horizontalScrollView.setOnScrollToIndexListen(new TPageHorizatalScrollView.OnScrollToIndexListen() {
            ...
            public void scrollToIndex(int index) {
                ...
                if (imageIds.get(currentIndex) instanceof Integer) {
                    animationIV.setImageResource((Integer) imageIds.get(currentIndex));
                } else {
                    Glide.with(getContext()).load((String) imageIds.get(currentIndex))
                ...
                pageControl.setCurrentPage(currentIndex);//指示器
            }
        });
...
//        });
    ...
    public void startTransform(final int state) {
        ...
        if (state == TScallImageView.STATE_TRANSFORM_IN) {
            ...
            gridViewBgView.setVisibility(INVISIBLE);//设置不可见？
            pageControl.setVisibility(INVISIBLE);//指示器设置不可见
        ...
        valueAnimator.addListener(new ValueAnimator.AnimatorListener() {
            ...
            public void onAnimationEnd(Animator animation) {
                ...
                if (mState == TScallImageView.STATE_TRANSFORM_IN) {
                    ...
                    pageControl.setVisibility(VISIBLE);//指示器可见
                    Handler handler = new Handler(){
                        @Override
                        public void handleMessage(Message msg) {
                            super.handleMessage(msg);
